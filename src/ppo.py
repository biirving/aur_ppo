...
class ppo():
	...
	def plot_episodic_returns(self, episodic_returns, window_size=10):
		smoothed_returns = self.moving_average(episodic_returns, window_size)
		plt.plot(episodic_returns, label='Episodic Returns')
		plt.plot(smoothed_returns, label=f'Moving Average (Window Size = {window_size})', color='red')
	...
	def train(self):
		...
		#for seed in range(len(self.envs.env_fns)):
		#	print(self.envs.env_fns[seed])
	#		self.envs.env_fns[seed].seed(seed=seed)
		policy_losses = []
		...
		self.plot_episodic_returns(np.array(self.total_returns))
