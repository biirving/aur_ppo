...
class bulletTrainer(policyTrainer):
    def __init__(self,  total_time_steps, num_env_steps, num_processes,  aug=False, do_pretraining=True):
        super().__init__()
        self.aug=aug
    ...
    def initialize_env(self, simulator, env_config, planner_config, gym_id):
        envs = EnvWrapper(self.num_processes, simulator, gym_id, env_config, planner_config)
        eval_envs = EnvWrapper(self.num_eval_processes, simulator, gym_id, env_config, planner_config)
        self.envs = envs
